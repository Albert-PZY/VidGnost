...
sequenceDiagram
    ...
    alt 7. 在线视频
        SUB->>SUB: 8. 用 yt-dlp 列出原字幕与自动翻译轨道
        SUB-->>ORC: 9. 返回字幕轨道列表
    else 10. 本地视频
...
- 对在线链接调用 `yt-dlp` 列出平台原字幕轨道；
- 列出平台自动翻译字幕轨道；
- 对没有字幕的在线视频自动触发 Whisper fallback；
- 对本地视频直接走 Whisper；
- 把轨道和字幕层状态写入 SQLite。
...
- 打通 `yt-dlp` 字幕轨道与 Whisper fallback。
...
- 用 `yt-dlp 字幕轨道优先 + Whisper 兜底 + 可选 LLM 翻译 + SQLite 学习域 + transcript-only QA` 构成新的主链路；
