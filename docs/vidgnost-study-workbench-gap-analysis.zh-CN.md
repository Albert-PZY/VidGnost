...
- YouTube `yt-dlp` 字幕轨道探测与自动翻译轨道列举；
- B 站登录态 AI 字幕获取；
- 字幕实时切换；
- 无字幕时 Whisper 兜底；
- 可选 LLM 翻译；
...
| 字幕来源 | 以 ASR 和后续 transcript 为主 | YouTube `yt-dlp` 字幕 / B 站登录态 AI 字幕优先，Whisper 兜底 | 缺少来源区分的字幕层 |
...
- VidGnost 保留本地优先底座；
- 在线视频改为 iframe 观看；
- YouTube 字幕以 `yt-dlp` 轨道优先，B 站字幕以登录态 AI 字幕优先，Whisper 兜底；
