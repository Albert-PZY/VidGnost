...
- [x] 建立任务处理页专项 profiling 方案，覆盖浏览器开发态、Electron 开发态、Electron 打包态。
- [x] 固定四个标准测试场景：长视频转写、长转写列表、长 VQA 回答、Markdown 编辑器 + Mermaid。
- [x] 记录首轮基线数据：构建体积、浏览器开发态任务页打开结果、DOM 节点数量、虚拟列表挂载数量、样本任务业务规模。
- [x] 在文档中补充每个场景的采样方法与验收阈值。
...
- [x] 对非关键 UI 刷新使用 `startTransition` 或等效的低优先级更新策略。
...
- [x] 保留 `content-visibility` 与 `contain` 作为虚拟列表之上的辅助层。
...
- [x] 为重型 Markdown 预处理建立 Worker 侧执行链路。
- [x] 为 Mermaid 预览建立可视区惰性渲染、空闲调度与 SVG 缓存链路。
...
- [x] 保障以下交互在任务运行期始终处于最高优先级：滚动、输入、拖拽分栏、视频播放控制、按钮点击。
- [x] 把阶段日志、次要 badge、非当前 tab 内容降级为后台更新。
- [x] 为分栏拖动期间的非关键渲染建立临时降频机制。
- [x] 对需要即时反馈但计算较重的区域统一采用占位态、渐进态、补全态三段式输出。

### P8 验收与回归

- [ ] 对比重构前后的 Electron 打包态性能数据。
- [x] 验证长视频任务运行期滚动与交互是否稳定在可接受范围。
- [x] 验证转写区、问答区、Markdown 编辑器在高压场景下不再出现明显输入延迟。
- [x] 验证视觉层收敛后是否仍符合项目既有 UI 风格。
- [x] 形成最终的性能验收报告并固化到文档。
