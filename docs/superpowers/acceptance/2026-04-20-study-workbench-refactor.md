...
| AC-002 | Study workbench payload can be read from a completed task without requiring VQA multimodal artifacts | API | Seeded task record with transcript and D-stage text artifacts | `GET /api/tasks/:taskId/study-pack` returns overview, highlights, themes, questions, quotes, subtitle tracks, translation status, study state, and task summary fields |
