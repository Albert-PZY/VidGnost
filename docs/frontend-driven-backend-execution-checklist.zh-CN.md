...
本清单定义当前前端页面与 `apps/api` 的联动基线，用于保证前端所有主视图都由 TS 后端真实驱动。
...
- 前端不承担任何服务端业务逻辑
- 所有运行态真值由 `apps/api` 返回
- 所有结构体定义以 `packages/contracts` 为唯一共享来源
- 所有运行时持久化写入 `storage/`
- 模型下载、Ollama 重启、模型迁移等动作必须以当前后端真实返回为准；当前不允许前端假定存在托管 `Ollama pull` 或自动迁移能力
