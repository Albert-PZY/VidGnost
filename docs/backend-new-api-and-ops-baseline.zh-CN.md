...
- Dense：ChromaDB `PersistentClient`
  - 路径：`backend-new/storage/vector-index/chroma-db`
  - 集合：`video_clips`
- Sparse：SQLite FTS5
  - 路径：`backend-new/storage/vector-index/sparse-fts5.sqlite3`
- 融合参数：`rrf_k=60`
- 默认召回参数：
  - `dense_top_k=80`
  - `sparse_top_k=120`
...
- Trace 日志路径：`backend-new/storage/event-logs/traces`
...
- 启动失败：
  ...
  - 检查端口 `8000` 占用
- 任务链路异常：
  - 查看 `backend-new/storage/event-logs/<task_id>.jsonl`
- VQA 追踪：
  - 查看 `backend-new/storage/event-logs/traces/*.jsonl`
- 配置回显异常：
  - 检查 `backend-new/storage/model_config.json`
  - 检查 `backend-new/storage/config.toml`
  - 检查 `backend-new/storage/models/catalog.json`
  - 检查 `backend-new/storage/config/ui_settings.json`
